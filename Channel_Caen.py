...
class Channel_Caen:
	def __init__(self, ch=3, ch_type='signal', verb=False):
		...
		self.offseted_adc_to_volts_cal = {'p0': 0, 'p1': np.divide(2, np.subtract(np.power(2, 14, dtype='float64'), 1.0, dtype='float64'), dtype='float64')}
	...
	def Correct_Base_Line(self, mean_adc, sigma_adc, settings):
		...
		self.base_line_u_adcs = self.Calculate_Universal_ADCs(self.ADC_to_Volts(mean_adc, settings.dig_bits), self.offseted_adc_to_volts_cal['p1'])
	...
	def ADC_to_Volts(self, adcs, nbits=14):
		return np.subtract(np.add(np.multiply(adcs, self.offseted_adc_to_volts_cal['p1'], dtype='f8'), np.divide(self.dc_offset_percent, 50.0, dtype='f8')), 1, dtype='f8')

	def Volts_to_ADC(self, volts):
		return RoundInt(np.multiply(np.divide(1, self.offseted_adc_to_volts_cal['p1'], dtype='f8'), np.add(np.subtract(volts, np.divide(self.dc_offset_percent, 50.0, dtype='f8'), dtype='f8'), 1, dtype='f8'), dtype='f8'), 'uint16')
